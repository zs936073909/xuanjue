...
(function(global){
  ...
  // 主题映射表：按问事类型调整机会、风险、行动、信号
  const XLR_THEME={
    '感情关系':{opps:['主动表达心意','制造相处机会','倾听对方需求'],risks:['避免情绪化争吵','忌翻旧账'],doAct:['真诚沟通','适度示好','观察对方反应'],signals:['看时宫是否生合月宫','注意空亡主缘分未至']},
    '事业合作':{opps:['寻找互补伙伴','推进关键项目','争取上级支持'],risks:['防范口舌是非','合同细节需看清'],doAct:['明确权责','稳步推进','保留书面记录'],signals:['时宫生月宫则贵人助','赤口宫慎签合约']},
    '学习考试':{opps:['制定复习计划','查漏补缺','请教师长'],risks:['贪多嚼不烂','临时抱佛脚'],doAct:['按部就班','专注基础','减少分心'],signals:['大安/速喜利学业','空亡主准备不足']},
    '出行移动':{opps:['选择吉时出发','提前规划路线'],risks:['空亡/赤口主阻滞','留连主延误'],doAct:['提前出发','检查交通工具','保持联络'],signals:['时宫为速喜/小吉利出行']},
    '签约交易':{opps:['条款清晰可签','速喜宫宜快速成交'],risks:['赤口主合同纠纷','空亡主落空'],doAct:['逐条核对','保留证据','必要时延后'],signals:['时宫生/比和日宫为吉']},
    '人际沟通':{opps:['化解误会','建立信任'],risks:['赤口主口舌','留连主纠缠'],doAct:['换位思考','委婉表达','避免指责'],signals:['时宫生月宫得人助']},
    '财务决策':{opps:['小吉宫小利财','速喜宫短期收益'],risks:['空亡主破财','赤口主损财'],doAct:['稳健理财','避免冲动投资','分散风险'],signals:['时宫五行生月宫为财来']},
    '健康倾向':{opps:['及时调整作息','寻求专业建议'],risks:['空亡主虚证','白虎/赤口主急症'],doAct:['注意休息','饮食清淡','及早就医'],signals:['大安主平稳，空亡需调养']},
    '失物寻找':{opps:['速喜宫可速得','小吉宫可寻回'],risks:['空亡主难寻','留连主被拖延'],doAct:['回忆最后位置','询问周边','扩大搜索'],signals:['时宫克日宫主难寻，生则易得']},
    '二选一决策':{opps:['比较两宫五行生克','选得时宫生者'],risks:['空亡宫选项多落空'],doAct:['列出优劣','参考三宫连锁','听取建议'],signals:['三宫相生则趋势顺畅']},
    '其他':{opps:['顺势而为','把握当下'],risks:['过犹不及'],doAct:['稳扎稳打','审时度势'],signals:['以时宫五行为主']}
  };
  function xiaoLiuRen(date,questionType){
    ...
    return{name:'小六壬',result:{month,day,time,detail:det,positions:[{k:'月',v:month},{k:'日',v:day},{k:'时',v:time}],chain,questionType},plain};
  }
  ...
  function meiHuaByInput(inputType,input,questionType,date){
    ...
    if(inputType==='random'){
      ...
      numbers={type:'random',up,down:dnIdx,dong:dongLine};
  ...
  function liuYaoByYaos(yaos,questionType,askInfo){
    ...
    if(yongMap.target==='世爻'){
      ...
      yongYao=yaoDetails.find(y=>y.liuQin===yongMap.target)||yaoDetails.find(y=>y.isShi);
    }
    // 空亡
    ...
    const dayRiRelation=dayRelation(dayGanWx,yongWx);
    const dongAny=dongCount>0;
    const plain={
      state:`六爻起得「${benGua.name}」${dongAny?`变「${bianGua.name}」`:''}，第${dongPositions.join('、')||'无'}爻动。世爻第${shiLine}爻，应爻第${yingLine}爻。卦宫${palace}(${palaceWx})。用神${yongMap.target}${yongYao?'·'+yongYao.liuQin+' '+yongYao.gz:''}。`,
      ...
      env:`卦宫五行${palaceWx}定六亲，日辰${baZi.day.gz}(${dayGanWx})${dayRiRelation}`,
      reviewDays:21,
      sources:[
        ...
        {type:'rule',desc:`日辰：${baZi.day.gz}(${dayGanWx})对用神${dayRiRelation}`}
      ]
    };
    return{name:'六爻',result:{
      benGua:benGua.name,bianGua:bianGua.name,yaos:yaoDetails,dongCount,dongPositions,
      palace,palaceWx,shiLine,yingLine,naJia,questionType,
      yongShen:{target:yongMap.target,yao:yongYao,desc:yongMap.desc},
      kongWang,yuePo:{zhi:yuePoZhi,lines:yuePoLines},dayRiRelation,
      dayGanWx:dayGanWx // 兼容旧字段
    },plain};
  }
  ...
  function tarot(date){
    ...
    const plain={
      ...
      reviewDays:30,
      sources:[
        {type:'rule',desc:'塔罗采用22张大阿卡纳三牌阵（过去-现在-未来）'},
        {type:'rule',desc:'随机抽取并随机正逆位'}
      ]
    };
  ...
  function baZiByBirth(birthInfo){
    ...
    const buildPillar=(name,p)=>{
      ...
      const cang=CANG_GAN[p.zhi].map(g=>({gan:g,wx:GAN_WX[g],shen:SHISHEN[dayGan][g]}));
      return{name:name,gz:p.gz,gan:p.gan,zhi:p.zhi,ganWx:GAN_WX[p.gan],zhiWx,ganShen:SHISHEN[dayGan][p.gan],cang};
    };
    ...
    if(!unknownHour){
      ...
      pillars.push({name:'时柱',gz:'未知',gan:'',zhi:'',ganWx:'',zhiWx:'',ganShen:'—',cang:[],unknown:true});
    }
    ...
    const daYun=computeDaYun(date,gender,bz.year.gan,monthGz60);
    // 当前流年、流月
    const now=new Date();
    const liuNian=Lunar.getBaZi(now).year;
    const liuYue=Lunar.getBaZi(now).month;
    const note=unknownHour?'时辰未知，本盘仅供参考':'此盘基于真实生辰';
    const plain={
      ...
      signals:[`日主${dayGan}${dayWx}`,`强弱：${dayStrong?'强':'弱'}`,`用神：${yongShen}`,`大运起运${daYun.startAge.toFixed(1)}岁`],
      ...
      sources:[
        ...
        {type:'rule',desc:`大运：${gender==='男'?'男命':'女命'}，年干${bz.year.gan}，${daYun.forward?'顺排':'逆排'}`},
        {type:'rule',desc:note}
      ]
    };
    return{name:'八字',result:{pillars,dayGan,dayWx,wxCount,wxCountAll,dayStrong,yongShen,wxStr,daYun,liuNian,liuYue,unknownHour,note},plain};
  }
  function computeDaYun(birthDate,gender,yearGan,monthGzIdx){
    ...
    const startAge=diffDays/3; // 三天折一岁
    const dy=[];
    let curIdx=monthGzIdx;
    for(let i=0;i<8;i++){
      const age=Math.round((startAge+i*10)*10)/10;
      curIdx=((curIdx%60)+60)%60;
      dy.push({age,gz:Lunar.gzFromIndex(curIdx),index:curIdx});
      curIdx=forward?curIdx+1:curIdx-1;
    }
    return{forward,startAge:Math.round(startAge*10)/10,pillars:dy};
  }
  function baZi(date){
